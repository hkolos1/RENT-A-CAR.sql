...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Dec 15, 2020 at 11:12 PM
...
DROP VIEW IF EXISTS `mostar_poslovnica`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mostar_poslovnica`  AS  select `p`.`ulica` AS `ulica`,`p`.`glavni_broj_tel` AS `glavni_broj_tel`,`ID_automobila` AS `ID_automobila`,`brend_automobila` AS `brend_automobila`,`deskripcija_automobila` AS `deskripcija_automobila`,`datum_kupovine_automobila` AS `datum_kupovine_automobila` from (`poslovnice` `p` join `automobili` `a`) where ((`ID_poslovnice` = `p`.`ID_poslovnice`) and (`p`.`grad` = 'Mostar')) ;
...
DROP VIEW IF EXISTS `skoda_rezervacije`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `skoda_rezervacije`  AS  select `brend_automobila` AS `brend_automobila`,`r`.`ID_rezervacija_automobila` AS `ID_rezervacija_automobila`,`r`.`cijena` AS `cijena`,`k`.`ID_kupac` AS `ID_kupac`,`k`.`broj_LK` AS `broj_LK`,`k`.`ime` AS `ime`,`k`.`prezime` AS `prezime` from ((`automobili` `a` join `rezervacija_automobila` `r` on((`r`.`ID_automobila` = `ID_automobila`))) join `kupci` `k`) where ((`r`.`ID_kupac` = `k`.`ID_kupac`) and (`brend_automobila` = 'Skoda')) ;
...
DROP VIEW IF EXISTS `suv_tipovi`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `suv_tipovi`  AS  select `t`.`ID_automobila` AS `ID_automobila`,`t`.`opis_automobila` AS `opis_automobila`,`brend_automobila` AS `brend_automobila` from (`tip_automobila` `t` join `automobili` `a`) where ((`ID_automobila` = `t`.`ID_automobila`) and (`t`.`opis_automobila` = 'SUV')) ;
